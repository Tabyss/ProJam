...
const getShapeClasses = (shape?: ShapeType): string => {
    ...
    const renderConnectorDot = (node: NodeProps, pos: pos) => {
        ...
        const isVisible = selectedNodeId === node.id || (!!draftEdge && isNodeHovered);
        ...
        return (
            <div
                ...
                onMouseUp={(e) => handleConnectorMouseUp && handleConnectorMouseUp(e, node, pos)}
    ...
    return (
        ...
        >
            ...
            {/* Handle Resize */}
            <div
                onMouseDown={(e) => selectedNodeId && handleResizeMouseDown(e, node)}
                className={`absolute -right-4 -bottom-4 size-3.5 ${selectedNodeId && 'cursor-nwse-resize'} bg-base-300 rounded-md z-10 ${selectedNodeId === node.id ? "opacity-50" : "opacity-0"}`}
            />
        </div>
    );
};
